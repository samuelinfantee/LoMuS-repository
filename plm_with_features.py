...
def _guess_lora_targets(hf_model: nn.Module):
    ...
    # BERT-style naming found in some ports / forks
...
class PLM_With_Features(nn.Module):
    """
    Same architecture/logic as before, but wraps an ESM2 model from Hugging Face.
    We keep the attribute name `self.bert` for minimal downstream changes.

    LoRA is injected *inside* self.bert so callers (train/test) don't change.
    """
    def __init__(self, feature_dim: int,
                 bert_model_name: str = 'facebook/esm2_t33_650M_UR50D',
        ...
        if use_lora:
            ...
            if not targets or not _targets_exist(self.bert, targets):
                targets = _guess_lora_targets(self.bert)
