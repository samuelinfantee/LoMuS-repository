...
X_TRAIN='X_train_std.npy'; X_VALID='X_valid_std.npy'
Y_TRAIN='y_train_aligned.npy'; Y_VALID='y_valid_aligned.npy'

BATCH_SIZE=8
DEFAULT_EPOCHS=10
GRAD_ACCUM_STEPS=4
WARMUP_RATIO=0.06
USE_TOKEN_FEATS=True

LM_NAME='facebook/esm2_t33_650M_UR50D'
OUTPUT_DIR='results/stability'
MAX_LEN=512; NUM_WORKERS=0
...
def train_one(device,
    ...
    train_dataset=StabilityWithFeaturesDataset(
        FASTA_TRAIN, X_TRAIN, Y_TRAIN, max_length=MAX_LEN,
        use_token_feats=USE_TOKEN_FEATS, wt_geom_path='WT_geom_stability.npz',
        wt_seq_path='data/stability/stability_wt.txt',
        esm_model_name=LM_NAME
    )
    valid_dataset=StabilityWithFeaturesDataset(
        FASTA_VALID, X_VALID, Y_VALID, max_length=MAX_LEN,
        use_token_feats=USE_TOKEN_FEATS, wt_geom_path='WT_geom_stability.npz',
        wt_seq_path='data/stability/stability_wt.txt',
        esm_model_name=LM_NAME
    )
    ...
    # --------- Two-LR optimizer (now CLI-configurable) ---------
    ...
    best_spearman=-1.0
    model.train()
    print(f"\n=== Training [stability] | lr_bert={lr_bert} lr_fusion={lr_fusion} "
          f"wd_bert={wd_bert} wd_fusion={wd_fusion} tag={run_tag or 'none'} ===")
    for epoch in range(epochs):
        ...
        if spearman>best_spearman:
            best_spearman=spearman; torch.save(model.state_dict(),out_path); print(f"💾 Saved best model → {out_path}")
        model.train()
...
def main():
    ...
    # learning rates & weight decays (for sweep)
...
X_TEST='X_test_std.npy'; Y_TEST='y_test_aligned.npy'
MAX_LEN=512; USE_TOKEN_FEATS=True
LM_NAME='facebook/esm2_t33_650M_UR50D'  # ⬅️ ESM2 backbone
OUTPUT_DIR='results/stability'
...
def main():
    ...
    test_dataset=StabilityWithFeaturesDataset(
        FASTA_TEST, X_TEST, Y_TEST, max_length=MAX_LEN,
        use_token_feats=USE_TOKEN_FEATS, wt_geom_path='WT_geom_stability.npz',
        wt_seq_path='data/stability/stability_wt.txt',
        esm_model_name=LM_NAME
    )
    ...
    # make an output dir for plots etc
